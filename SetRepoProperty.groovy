...
// curl -X POST -v -u admin:Admin@123 "http://localhost:8082/artifactory/api/plugins/execute/setPropertyValues?
// params=propertyName=artifact.developer;propertyValue=arunkg;repo=docker-local-akg"

executions {
    setPropertyValues() { params ->
        ...
        setPropertyValues(propertyName, propertyValue, repo, dryRun)
    }
}

private def setPropertyValues(propertyName, propertyValue, repo, dryRun) {
    log.info "Searching for files that are missing the property $propertyName in $repo"

    def count = 0
    def aql = "items.find({\"repo\":{ \"\$eq\":\""+ repo +"\" }})"
